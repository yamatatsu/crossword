  ...
  return (
    <div>
      ...
      <ol className="space-y-1">
        {clues.map((clue) => {
          const isSelected = clue.id === selectedClueId && direction === selectedDirection;
          return (
            <li
              key={clue.id}
              className={`text-sm cursor-pointer rounded px-2 py-1 ${
                isSelected ? "bg-blue-100 font-medium" : "hover:bg-muted"
              }`}
              onClick={() => onSelectClue(clue.id, direction)}
              onKeyDown={() => {}}
            >
              <span className="font-semibold mr-1">{clue.id}.</span>
              {clue.clue}
            </li>
          );
        })}
...
export function ClueList({ clues, selectedClueId, selectedDirection, onSelectClue }: Props) {
  return (
