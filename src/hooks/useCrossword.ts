...
export function useCrossword(puzzle: PuzzleData) {
  const [state, dispatch] = useReducer(crosswordReducer, puzzle, initializeState);
  ...
  const selectClue = useCallback((clueId: number, direction: Direction) => {
  ...
  const move = useCallback((direction: CrosswordAction & { type: "MOVE" }) => {
    dispatch(direction);
  }, []);
  ...
  const clues = useMemo(
    () => ({
      across: state.puzzle.words
        ...
        .sort((a, b) => a.id - b.id),
      down: state.puzzle.words.filter((w) => w.direction === "down").sort((a, b) => a.id - b.id),
    }),
    [state.puzzle.words],
  );
