...
import { type CrosswordAction, crosswordReducer, initializeState } from "../crosswordReducer";
...
function applyActions(puzzle: PuzzleData, actions: CrosswordAction[]) {
...
describe("SET_CELL", () => {
  ...
  it("converts letter to uppercase", () => {
    const state = applyActions(testPuzzle, [{ type: "SET_CELL", row: 0, col: 0, letter: "c" }]);
    expect(state.grid[0][0]?.letter).toBe("C");
  });
...
describe("SELECT_CELL", () => {
  it("selects a white cell", () => {
    const state = applyActions(testPuzzle, [{ type: "SELECT_CELL", row: 0, col: 1 }]);
    expect(state.selectedCell).toEqual({ row: 0, col: 1 });
  });

  it("ignores selection of black cells", () => {
    const state = applyActions(testPuzzle, [{ type: "SELECT_CELL", row: 4, col: 4 }]);
    expect(state.selectedCell).toBeNull();
  });
  ...
  it("sets the clue id for the selected word", () => {
    const state = applyActions(testPuzzle, [{ type: "SELECT_CELL", row: 0, col: 0 }]);
...
describe("SELECT_CLUE", () => {
  it("selects the first cell of the clue word", () => {
    const state = applyActions(testPuzzle, [{ type: "SELECT_CLUE", clueId: 2, direction: "down" }]);
    expect(state.selectedCell).toEqual({ row: 0, col: 0 });
    expect(state.selectedDirection).toBe("down");
...
describe("TOGGLE_DIRECTION", () => {
  ...
  it("does nothing without a selected cell", () => {
    const state = applyActions(testPuzzle, [{ type: "TOGGLE_DIRECTION" }]);
    expect(state.selectedDirection).toBe("across");
  });
});
