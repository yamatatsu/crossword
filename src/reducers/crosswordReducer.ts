...
import type { CellState, CrosswordState, Direction, PuzzleData } from "@/types/crossword";
...
export function initializeState(puzzle: PuzzleData): CrosswordState {
  const grid: (CellState | null)[][] = Array.from({ length: puzzle.size.rows }, () =>
...
function selectCellAndClue(
  ...
  const actualDirection = word ? direction : direction === "across" ? "down" : "across";
  const actualWord = word ?? getWordAtCell(state, row, col, actualDirection);
  ...
  switch (action.type) {
    ...
    case "DELETE_CELL": {
      ...
      if (cell.letter === "") {
        ...
        const prev = getPrevCell(state, row, col, state.selectedDirection);
    ...
    case "SELECT_CELL": {
      ...
      if (state.selectedCell?.row === row && state.selectedCell?.col === col) {
        const newDirection: Direction = state.selectedDirection === "across" ? "down" : "across";
    ...
    case "SELECT_CLUE": {
      const word = state.puzzle.words.find(
        (w) => w.id === action.clueId && w.direction === action.direction,
      );
    ...
    case "TOGGLE_DIRECTION": {
      if (!state.selectedCell) return state;
      const newDir: Direction = state.selectedDirection === "across" ? "down" : "across";
      return selectCellAndClue(state, state.selectedCell.row, state.selectedCell.col, newDir);
    }
      ...
      for (const { row, col } of cells) {
        ...
        if (cell && cell.letter !== "") {
          newGrid[row][col] = {
            ...cell,
            checked: true,
            correct: cell.letter === solution.get(`${row},${col}`),
          };
        }
      }
